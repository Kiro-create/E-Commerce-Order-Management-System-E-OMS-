...
| **Plan Shipment**               | When customer orders are marked “Ready for Shipment,” the system automatically assigns each order to the optimal courier based on rules (destination zone, weight, cost, SLA performance, courier availability). The logistics staff reviews the automatically assigned shipments, handles exceptions, and confirms the final shipping manifest for courier pickup. |
